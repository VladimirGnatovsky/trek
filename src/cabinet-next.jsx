...
function EntryModal({ initial, month, categoryRules, onClose, onSave, onScan }) {
  ...
  return <Modal onClose={onClose}><span className="tc-kicker">{initial ? "EDIT TRANSACTION" : "NEW TRANSACTION"}</span><h2>{initial ? "Update entry" : "Log an entry"}</h2><form className="tc-form tn-form-grid" onSubmit={async (event) => { event.preventDefault(); const amount = Number(String(form.amount).replace(",", ".")); if (!form.merchant.trim() || amount <= 0) return; setBusy(true); const ok = await onSave({ ...form, amount, tags: typeof form.tags === "string" ? form.tags.split(",").map((tag) => tag.trim()).filter(Boolean) : form.tags }); setBusy(false); if (ok) onClose(); }}>
    ...
    <label>Merchant or source<input autoFocus value={form.merchant} onChange={(event) => { const merchant = event.target.value; const category = !categoryTouched ? suggestedCategory(merchant, categoryRules) : null; setForm((current) => ({ ...current, merchant, ...(category ? { category } : {}) })); }} /></label>
    ...
    {form.type === "expense" && <label>Category<select value={form.category} onChange={(event) => { setCategoryTouched(true); set("category", event.target.value); }}>{CATEGORIES.map((item) => <option key={item}>{item}</option>)}</select></label>}
...
function SettingsPage({ user, profileName, avatarUrl, currency, fallbackBudget, plan, privacy, notificationsEnabled, widgets, canExport, transactions, budgets, categoryBudgets, goals, recurring, cryptoHoldings, isAdmin, onProfile, onAvatar, onCurrency, onPrivacy, onNotifications, onWidgets, onPortal, onUpgrade, onAdmin, onDeleteAccount, onRestoreBackup, nativeApp = false }) {
  ...
  return <section className="tc-page"><div className="tc-page-head"><div><span>ACCOUNT</span><h2>Settings</h2><p>Control your profile, privacy, currency and membership.</p></div></div><div className="tc-settings">
    <div className="tn-settings-column">
      <section className="tc-panel"><span>PROFILE</span><div className="tc-profile-row">{avatarUrl ? <img className="tc-profile-avatar" src={avatarUrl} alt="Profile" /> : <span className="tc-profile-avatar">{profileName.charAt(0).toUpperCase()}</span>}<div><h3>{profileName}</h3><p>{user.email}</p></div></div><div className="tn-settings-actions"><label className="tc-upload"><Upload size={14} /> Upload photo<input type="file" accept="image/png,image/jpeg,image/webp" onChange={async (event) => { const file = event.target.files?.[0]; if (!file) return; try { setBusy(true); await onAvatar(file); } catch (uploadError) { setError(uploadError.message); } finally { setBusy(false); } }} /></label></div><form className="tc-inline tn-profile-editor" onSubmit={saveName}><input aria-label="Profile name" value={name} onChange={(event) => setName(event.target.value)} /><button disabled={busy || !name.trim()}>{busy ? "Saving…" : "Save name"}</button></form>{error && <p className="tc-form-error">{error}</p>}</section>
...
export default function Cabinet({ onExit, onSignOut, user, onProfileUpdate, nativeApp = false }) {
  ...
  return <div className={`tc-app${nativeApp ? " tc-native" : ""}`}><aside className="tc-side"><button className="tc-brand" onClick={onExit}><i><ArrowUpRight size={17} /></i> Trek</button><small>PERSONAL SPACE</small>{NAV.map(([id, label, Icon]) => <button key={id} className={`${view === id ? "on" : ""}${lockedView(id) ? " locked" : ""}`} onClick={() => changeView(id)}><Icon size={17} /> {label}{lockedView(id) ? " · Plus" : ""}</button>)}<div className="tc-side-bottom"><button onClick={() => setModal("pricing")}><CreditCard size={17} /> {plan} plan</button><button onClick={() => setView("settings")}><Settings size={17} /> Settings</button>{!nativeApp && <button onClick={onExit}><ArrowLeft size={16} /> Back to home</button>}<button onClick={onSignOut}><LogOut size={17} /> Sign out</button></div></aside>
    ...
    {modal === "entry" && <EntryModal initial={editing ? { ...editing, amount: String(editing.amount), tags: editing.tags || [] } : null} month={month} categoryRules={merchantRules} onClose={() => setModal(null)} onSave={saveTransaction} onScan={scanReceipt} />}
