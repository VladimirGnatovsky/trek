...
function AccountsPage({ accounts, transfers, currency, hidden, onAdd, onEdit, onTransfer, onArchive, locale }) {
...
export default function Cabinet({ onExit, onSignOut, user, onProfileUpdate, locale = "en", onLocale, theme = "dark", onTheme, nativeApp = false }) {
  ...
  const [view, setView] = useState("overview"); const [month, setMonth] = useState(monthKey()); const [modal, setModal] = useState(null); const [editing, setEditing] = useState(null); const [entryDraft, setEntryDraft] = useState(null); const [editingAccount, setEditingAccount] = useState(null); const [editingGoal, setEditingGoal] = useState(null); const [editingRule, setEditingRule] = useState(null); const [editingCrypto, setEditingCrypto] = useState(null);
  ...
  else if (view === "accounts") page = <AccountsPage accounts={accountBalances} transfers={transfers} currency={currency} hidden={privacy} locale={locale} onAdd={() => { setEditingAccount(null); setModal("account"); }} onEdit={(account) => { setEditingAccount(account); setModal("account"); }} onTransfer={() => setModal("transfer")} onArchive={archiveAccount} />;
