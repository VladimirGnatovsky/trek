...
import { buildFinancialTimeline, buildSmartInsights, buildWeeklyReport, calculateAccountBalances, calculateNoSpendStreak, detectRecurringCandidates } from "../lib/product-intelligence.mjs";
...
function EntryModal({ initial, isEditing = false, month, categoryRules, accounts, onClose, onSave, onScan, copy, locale }) {
  ...
  return <Modal onClose={onClose}><span className="tc-kicker">{isEditing ? copy.edit : copy.fresh}</span><h2>{isEditing ? copy.update : copy.log}</h2><form className="tc-form tn-form-grid" onSubmit={async (event) => { event.preventDefault(); const amount = Number(String(form.amount).replace(",", ".")); if (!form.merchant.trim() || amount <= 0) return; setBusy(true); const ok = await onSave({ ...form, amount, tags: typeof form.tags === "string" ? form.tags.split(",").map((tag) => tag.trim()).filter(Boolean) : form.tags }); setBusy(false); if (ok) onClose(); }}>
    ...
    <label>{copy.tags}<input placeholder="work, travel" value={Array.isArray(form.tags) ? form.tags.join(", ") : form.tags} onChange={(event) => set("tags", event.target.value)} /></label>
...
function AccountsPage({ accounts, transfers, currency, hidden, onAdd, onTransfer, onArchive, locale }) {
  ...
  return <section className="tc-page tn-accounts-page">
    ...
    <div className="tn-account-grid">{active.map((account) => { const Icon = ACCOUNT_ICONS[account.kind] || Wallet; return <article className={`tc-panel tn-account-card ${account.kind}`} key={account.id}><header><span><Icon size={18} /></span>{account.kind !== "crypto" && <button onClick={() => onArchive(account.id)} aria-label={ui(locale, "Archive account")}><Trash2 size={14} /></button>}</header><small>{ui(locale, account.kind.toUpperCase())}</small><h3>{ui(locale, account.name)}</h3><b>{money(account.balance, currency, hidden)}</b><p>{account.kind === "crypto" ? ui(locale, "Synced with your crypto portfolio") : ui(locale, "Includes entries and transfers")}</p></article>; })}</div>
    <section className="tc-panel tn-transfer-list"><header><div><span>{ui(locale, "RECENT TRANSFERS")}</span><h3>{ui(locale, "Money movement")}</h3></div></header>{transfers.length ? transfers.slice(0, 12).map((item) => <article key={item.id}><span><ArrowRightLeft size={15} /></span><div><b>{accountName(item.fromAccountId)} <ChevronRight size={13} /> {accountName(item.toAccountId)}</b><small>{item.date}{item.note ? ` · ${item.note}` : ""}</small></div><strong>{money(item.amount, currency, hidden)}</strong></article>) : <div className="tn-account-empty"><ArrowRightLeft size={22} /><b>{ui(locale, "No transfers yet")}</b><p>{ui(locale, "Transfers change account balances but never affect spending reports.")}</p></div>}</section>
    {archived.length > 0 && <section className="tc-panel tn-archived-accounts"><span>{ui(locale, "ARCHIVED ACCOUNTS")}</span>{archived.map((account) => <div key={account.id}><b>{ui(locale, account.name)}</b><button onClick={() => onArchive(account.id, false)}>{ui(locale, "Restore")}</button></div>)}</section>}
  </section>;
}

function AccountModal({ currency, onClose, onSave, locale }) {
  const [form, setForm] = useState({ name: "", kind: "bank", openingBalance: "" });
  const [busy, setBusy] = useState(false);
  return <Modal onClose={onClose}><span className="tc-kicker">{ui(locale, "NEW ACCOUNT")}</span><h2>{ui(locale, "Add a place for your money.")}</h2><p className="tc-modal-copy">{ui(locale, "Use the opening balance once; future changes come from transactions and transfers.")}</p><form className="tc-form" onSubmit={async (event) => { event.preventDefault(); if (!form.name.trim()) return; setBusy(true); const ok = await onSave({ ...form, openingBalance: Number(String(form.openingBalance || 0).replace(",", ".")) }); setBusy(false); if (ok) onClose(); }}><label>{ui(locale, "Account name")}<input autoFocus value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder={ui(locale, "Main card")} /></label><label>{ui(locale, "Account type")}<select value={form.kind} onChange={(event) => setForm({ ...form, kind: event.target.value })}><option value="cash">{ui(locale, "Cash")}</option><option value="bank">{ui(locale, "Bank card")}</option><option value="savings">{ui(locale, "Savings")}</option></select></label><label>{ui(locale, "Opening balance")} ({currency})<input inputMode="decimal" value={form.openingBalance} onChange={(event) => setForm({ ...form, openingBalance: event.target.value })} placeholder="0" /></label><button className="tc-action" disabled={busy}>{ui(locale, busy ? "Saving…" : "Create account")}</button></form></Modal>;
}

function TransferModal({ accounts, currency, onClose, onSave, locale }) {
  const available = accounts.filter((item) => !item.archived && item.kind !== "crypto");
  const [form, setForm] = useState({ fromAccountId: available[0]?.id || "", toAccountId: available[1]?.id || "", amount: "", fee: "", date: iso(new Date()), note: "" });
  const [busy, setBusy] = useState(false);
...
function AdminModal({ onClose, onLookup, onGrant }) {
  ...
  return <Modal onClose={onClose}><span className="tc-kicker">TREK ADMIN</span><h2>Manage membership.</h2><p className="tc-modal-copy">Find a registered account and assign access without editing the database manually.</p><form className="tc-form tn-admin-search" onSubmit={lookup}><label>User email<input autoFocus type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="user@example.com" /></label><button className="tc-action" disabled={busy || !email.trim()}>{busy ? "Searching…" : "Find account"}</button></form>{error && <p className="tc-form-error">{error}</p>}{account && <section className="tn-admin-account"><div><span>{(account.full_name || account.email).charAt(0).toUpperCase()}</span><div><b>{account.full_name || "Trek member"}</b><small>{account.email}</small></div><strong>{account.plan}</strong></div><p>Choose the access level. Manual grants do not create or charge a Stripe subscription.</p><div>{["Start","Plus","Lifetime"].map((item) => <button key={item} className={account.plan === item ? "on" : ""} disabled={busy || account.plan === item} onClick={async () => { setBusy(true); setError(""); try { setAccount(await onGrant(account.email, item)); } catch (grantError) { setError(grantError.message); } finally { setBusy(false); } }}>{item}</button>)}</div></section>}</Modal>;
}
...
function DeleteAccountModal({ onClose, onDelete }) {
  ...
  return <Modal onClose={busy ? () => {} : onClose} locked={busy}><span className="tc-kicker tn-danger">DANGER ZONE</span><h2>Delete your Trek account?</h2><p className="tc-modal-copy">This permanently removes your login and all connected Trek data. Active Stripe subscriptions must be cancelled separately in the billing portal.</p><form className="tc-form" onSubmit={async (event) => { event.preventDefault(); if (confirmation !== "DELETE") return; setBusy(true); setError(""); try { await onDelete(confirmation); } catch (deleteError) { setError(deleteError.message || "Account could not be deleted."); setBusy(false); } }}><label>Type DELETE to confirm<input autoFocus autoComplete="off" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} /></label>{error && <p className="tc-form-error">{error}</p>}<div className="tn-delete-actions"><button type="button" onClick={onClose} disabled={busy}>Keep my account</button><button className="tn-delete-confirm" disabled={busy || confirmation !== "DELETE"}>{busy ? "Deleting…" : "Delete permanently"}</button></div></form></Modal>;
}
...
function SettingsPage({ user, profileName, avatarUrl, currency, fallbackBudget, plan, privacy, notificationsEnabled, widgets, canExport, transactions, budgets, categoryBudgets, goals, recurring, cryptoHoldings, isAdmin, onProfile, onAvatar, onCurrency, onPrivacy, onNotifications, onWidgets, onPortal, onUpgrade, onAdmin, onDeleteAccount, onRestoreBackup, locale, onLocale, theme, onTheme, copy, nativeApp = false }) {
  ...
  return <section className="tc-page"><div className="tc-page-head"><div><span>{copy.eyebrow}</span><h2>{copy.title}</h2><p>{copy.intro}</p></div></div><div className="tc-settings">
    <div className="tn-settings-column">
      <section className="tc-panel"><span>{copy.profile}</span><div className="tc-profile-row">{avatarUrl ? <img className="tc-profile-avatar" src={avatarUrl} alt="Profile" /> : <span className="tc-profile-avatar">{profileName.charAt(0).toUpperCase()}</span>}<div><h3>{profileName}</h3><p>{user.email}</p></div></div><div className="tn-settings-actions"><label className="tc-upload"><Upload size={14} /> {copy.upload}<input type="file" accept="image/png,image/jpeg,image/webp" onChange={async (event) => { const file = event.target.files?.[0]; if (!file) return; try { setBusy(true); await onAvatar(file); } catch (uploadError) { setError(uploadError.message); } finally { setBusy(false); } }} /></label></div><form className="tc-inline tn-profile-editor" onSubmit={saveName}><input aria-label="Profile name" value={name} onChange={(event) => setName(event.target.value)} /><button disabled={busy || !name.trim()}>{busy ? copy.saving : copy.save}</button></form>{error && <p className="tc-form-error">{error}</p>}</section>
...
export default function Cabinet({ onExit, onSignOut, user, onProfileUpdate, locale = "en", onLocale, theme = "dark", onTheme, nativeApp = false }) {
  ...
  const [view, setView] = useState("overview"); const [month, setMonth] = useState(monthKey()); const [modal, setModal] = useState(null); const [editing, setEditing] = useState(null); const [entryDraft, setEntryDraft] = useState(null);
  ...
  const [accounts, setAccounts] = useState([]); const [transfers, setTransfers] = useState([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!active) return;
      const [profileResult, settingsResult, transactionResult, subscriptionResult, budgetResult, categoryResult, goalResult, recurringResult, cryptoResult, accountResult, transferResult] = results;
  ...
  const createAccount = async (form) => { const result = await supabase.from("financial_accounts").insert({ user_id: user.id, name: form.name.trim(), kind: form.kind, currency, opening_balance: form.openingBalance }).select().single(); if (result.error) return setError(result.error); setAccounts((current) => [...current, fromAccount(result.data)]); showToast(ui(locale, "Account created.")); return true; };
  ...
  const createTransfer = async (form) => { const result = await supabase.from("account_transfers").insert({ user_id: user.id, from_account_id: form.fromAccountId, to_account_id: form.toAccountId, amount: form.amount, fee: form.fee, occurred_on: form.date, note: form.note || "" }).select().single(); if (result.error) return setError(result.error); setTransfers((current) => [fromTransfer(result.data), ...current]); showToast(ui(locale, "Transfer saved.")); return true; };
  ...
  const confirmStatement = async (rows, fileName, months) => {
    ...
    const payload = rows.map((row) => ({ user_id: user.id, merchant: row.merchant, category: row.type === "income" ? "Other" : row.category, entry_type: row.type, amount: row.amount, occurred_on: row.date, account_id: defaultAccountId,
      note: row.originalCurrency === currency ? `Imported from ${source}` : `Imported from ${source} · ${row.originalAmount} ${row.originalCurrency} at the historical official rate`, tags: ["imported", "bank-statement"], needs_review: false,
  ...
  else if (view === "accounts") page = <AccountsPage accounts={accountBalances} transfers={transfers} currency={currency} hidden={privacy} locale={locale} onAdd={() => setModal("account")} onTransfer={() => setModal("transfer")} onArchive={archiveAccount} />;
  ...
  return <div className={`tc-app${nativeApp ? " tc-native" : ""}`}><aside className="tc-side"><button className="tc-brand" onClick={onExit}><i><ArrowUpRight size={17} /></i> Trek</button><small>{copy.shell.personal}</small>{localizedNav.map(([id, label, Icon]) => <button key={id} className={`${view === id ? "on" : ""}${lockedView(id) ? " locked" : ""}`} onClick={() => changeView(id)}><Icon size={17} /> {label}{lockedView(id) ? " · Plus" : ""}</button>)}<div className="tc-side-bottom"><button onClick={() => setModal("pricing")}><CreditCard size={17} /> {plan} {copy.shell.plan}</button><button onClick={() => setView("settings")}><Settings size={17} /> {copy.shell.settings}</button>{!nativeApp && <button onClick={onExit}><ArrowLeft size={16} /> {copy.shell.back}</button>}<button onClick={onSignOut}><LogOut size={17} /> {copy.shell.signout}</button></div></aside>
    ...
    {modal === "account" && <AccountModal currency={currency} locale={locale} onClose={() => setModal(null)} onSave={createAccount} />}
    {modal === "transfer" && <TransferModal accounts={accounts} currency={currency} locale={locale} onClose={() => setModal(null)} onSave={createTransfer} />}
    ...
    {modal === "crypto" && <CryptoModal currency={currency} locale={locale} onClose={() => setModal(null)} onSave={saveCrypto} />}
    ...
    {modal === "admin" && <AdminModal onClose={() => setModal(null)} onLookup={lookupAdminUser} onGrant={grantMembership} />}
    {modal === "delete-account" && <DeleteAccountModal onClose={() => setModal(null)} onDelete={deleteAccount} />}
