...
function Empty({ icon: Icon, title, copy }) { return <div className="tc-empty"><Icon size={26} /><b>{title}</b><span>{copy}</span></div>; }
...
function PricingModal({ plan, user, onClose, onCheckout }) {
  ...
  return <Modal onClose={onClose} wide><span className="tc-kicker">MEMBERSHIP</span><h2>Choose your Trek mode.</h2><div className="tc-prices">{choices.map(([name, price, features]) => <article key={name} className={plan === name ? "selected" : ""}><span>{name}</span><h3>{price}</h3><ul>{features.map((feature) => <li key={feature}><Check size={13} /> {feature}</li>)}</ul><button disabled={plan === name || name === "Start"} onClick={() => onCheckout(name, user)}>{plan === name ? "Current plan" : name === "Start" ? "Included" : "Continue to payment"}</button></article>)}</div></Modal>;
}
