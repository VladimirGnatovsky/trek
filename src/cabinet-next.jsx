...
function AutomationPage({ rules, canUse, onAddPreset, onNew, onEdit, onToggle, onDelete, onUpgrade, locale }) {
  ...
  return <section className="tc-page">
    ...
    <section className="tc-panel tn-automation-presets"><header><div><span>{ui(locale, "STARTER RULES")}</span><h3>{ui(locale, "Add a useful rule in one click")}</h3></div></header><div>{AUTOMATION_PRESETS.map((preset) => { const added = rules.some((rule) => rule.name === preset.name); return <article key={preset.name}><span><Workflow size={17} /></span><div><b>{ui(locale, preset.name)}</b><small>{condition(preset)} → {action(preset)}</small></div><button className={added ? "tn-preset-added" : ""} disabled={added} aria-label={ui(locale, added ? "Added" : "Add rule")} title={ui(locale, added ? "Added" : "Add rule")} onClick={() => canUse ? onAddPreset(preset) : onUpgrade}>{added ? <Check size={19} /> : ui(locale, "Add rule")}</button></article>; })}</div></section>
