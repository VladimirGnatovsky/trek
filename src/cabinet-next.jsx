...
function EntryModal({ initial, month, categoryRules, onClose, onSave, onScan, copy }) {
  ...
  return <Modal onClose={onClose}><span className="tc-kicker">{initial ? copy.edit : copy.fresh}</span><h2>{initial ? copy.update : copy.log}</h2><form className="tc-form tn-form-grid" onSubmit={async (event) => { event.preventDefault(); const amount = Number(String(form.amount).replace(",", ".")); if (!form.merchant.trim() || amount <= 0) return; setBusy(true); const ok = await onSave({ ...form, amount, tags: typeof form.tags === "string" ? form.tags.split(",").map((tag) => tag.trim()).filter(Boolean) : form.tags }); setBusy(false); if (ok) onClose(); }}>
    ...
    {form.type === "expense" && <label>{copy.category}<select value={form.category} onChange={(event) => { setCategoryTouched(true); set("category", event.target.value); }}>{CATEGORIES.map((item) => <option key={item}>{item}</option>)}</select></label>}
...
function StatementImportModal({ currency, onClose, onAnalyze, onConfirm }) {
  ...
  return <Modal onClose={onClose} wide><span className="tc-kicker">LIFETIME · SMART IMPORT</span><h2>Import a bank statement.</h2><p className="tc-modal-copy">Upload CSV or PDF, review the detected categories, then add the selected rows. Foreign amounts use the historical official exchange rate available for each transaction date.</p>
...
function SettingsPage({ user, profileName, avatarUrl, currency, fallbackBudget, plan, privacy, notificationsEnabled, widgets, canExport, transactions, budgets, categoryBudgets, goals, recurring, cryptoHoldings, isAdmin, onProfile, onAvatar, onCurrency, onPrivacy, onNotifications, onWidgets, onPortal, onUpgrade, onAdmin, onDeleteAccount, onRestoreBackup, locale, onLocale, copy, nativeApp = false }) {
  ...
  return <section className="tc-page"><div className="tc-page-head"><div><span>{copy.eyebrow}</span><h2>{copy.title}</h2><p>{copy.intro}</p></div></div><div className="tc-settings">
    <div className="tn-settings-column">
      ...
      {isAdmin && <section className="tc-panel tn-admin-card"><span>ADMINISTRATION</span><h3>Membership access</h3><p>Find users and grant Start, Plus or Lifetime access from a protected server endpoint.</p><button onClick={onAdmin}><ShieldCheck size={15} /> Open admin tools</button></section>}
    </div>
    <div className="tn-settings-column">
      <section className="tc-panel"><span>{copy.display}</span><small className="tn-setting-label">{copy.language}</small><LanguageSwitch locale={locale} onChange={onLocale} className="tn-language-switch" /><h3>{currency}</h3><div className="tc-choice">{["EUR", "USD", "PLN", "UAH"].map((item) => <button key={item} className={currency === item ? "on" : ""} onClick={() => onCurrency(item)}>{item}</button>)}</div><div className="tn-setting-stack"><button onClick={() => onPrivacy(!privacy)}>{privacy ? <Eye size={15} /> : <EyeOff size={15} />} {privacy ? "Show amounts" : "Hide amounts"}</button><button onClick={() => onNotifications(!notificationsEnabled)}><Bell size={15} /> {notificationsEnabled ? "Budget alerts on" : "Budget alerts off"}</button></div></section>
      <section className="tc-panel"><span>YOUR DATA</span><h3>Backup and restore</h3><p>{canExport ? "Download or restore transactions, goals, recurring items and crypto positions." : "Cloud backup tools are available with Plus or Lifetime."}</p><div className="tn-setting-stack"><button onClick={canExport ? exportData : onUpgrade}><Download size={15} /> {canExport ? "Export data" : "Upgrade to export"}</button>{canExport && <button onClick={onRestoreBackup}><FileUp size={15} /> Restore backup</button>}</div></section>
      <section className="tc-panel tn-danger-zone"><span>ACCOUNT REMOVAL</span><h3>Delete account</h3><p>Permanently erase your Trek account and stored financial data.</p><button onClick={onDeleteAccount}><Trash2 size={15} /> Delete my account</button></section>
    </div>
  </div></section>;
}

export default function Cabinet({ onExit, onSignOut, user, onProfileUpdate, locale = "en", onLocale, nativeApp = false }) {
  ...
  return <div className={`tc-app${nativeApp ? " tc-native" : ""}`}><aside className="tc-side"><button className="tc-brand" onClick={onExit}><i><ArrowUpRight size={17} /></i> Trek</button><small>{copy.shell.personal}</small>{localizedNav.map(([id, label, Icon]) => <button key={id} className={`${view === id ? "on" : ""}${lockedView(id) ? " locked" : ""}`} onClick={() => changeView(id)}><Icon size={17} /> {label}{lockedView(id) ? " · Plus" : ""}</button>)}<div className="tc-side-bottom"><button onClick={() => setModal("pricing")}><CreditCard size={17} /> {plan} {copy.shell.plan}</button><button onClick={() => setView("settings")}><Settings size={17} /> {copy.shell.settings}</button>{!nativeApp && <button onClick={onExit}><ArrowLeft size={16} /> {copy.shell.back}</button>}<button onClick={onSignOut}><LogOut size={17} /> {copy.shell.signout}</button></div></aside>
    ...
    {modal === "entry" && <EntryModal initial={editing ? { ...editing, amount: String(editing.amount), tags: editing.tags || [] } : null} month={month} categoryRules={merchantRules} copy={copy.entry} onClose={() => setModal(null)} onSave={saveTransaction} onScan={scanReceipt} />}
