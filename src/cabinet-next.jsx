...
function SettingsPage({ user, profileName, avatarUrl, currency, plan, privacy, notificationsEnabled, widgets, canExport, transactions, goals, recurring, cryptoHoldings, isAdmin, onProfile, onAvatar, onCurrency, onPrivacy, onNotifications, onWidgets, onPortal, onUpgrade, onAdmin, onDeleteAccount, nativeApp = false }) {
...
export default function Cabinet({ onExit, onSignOut, user, onProfileUpdate, nativeApp = false }) {
  ...
  else page = <SettingsPage user={user} profileName={profileName} avatarUrl={avatarUrl} currency={currency} plan={plan} privacy={privacy} notificationsEnabled={notificationsEnabled} widgets={widgets} canExport={plus} transactions={transactions} goals={goals} recurring={recurring} cryptoHoldings={cryptoHoldings} isAdmin={isAdmin} onProfile={updateProfile} onAvatar={uploadAvatar} onCurrency={(value) => saveSettings({ currency: value })} onPrivacy={(value) => saveSettings({ privacy_mode: value })} onNotifications={(value) => saveSettings({ notifications_enabled: value })} onWidgets={(value) => saveSettings({ dashboard_widgets: value })} onPortal={billingPortal} onUpgrade={() => setModal("pricing")} onAdmin={() => setModal("admin")} onDeleteAccount={() => setModal("delete-account")} nativeApp={nativeApp} />;
