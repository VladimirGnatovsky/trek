...
function AutomationPage({ rules, canUse, onAddPreset, onNew, onToggle, onDelete, onUpgrade, locale }) {
...
function AutomationModal({ onClose, onSave, locale }) {
  const [form, setForm] = useState({ name: "", merchantContains: "", amountAbove: "", type: "any", actionType: "keep", category: "", markReview: false });
  const [busy, setBusy] = useState(false);
  return <Modal onClose={onClose}><span className="tc-kicker">{ui(locale, "NEW AUTOMATION")}</span><h2>{ui(locale, "Create an automation rule.")}</h2><p className="tc-modal-copy">{ui(locale, "New transactions will apply every matching active rule.")}</p><form className="tc-form" onSubmit={async (event) => { event.preventDefault(); if (!form.name.trim() || (!form.merchantContains.trim() && !(Number(form.amountAbove) > 0))) return; setBusy(true); const ok = await onSave({ ...form, amountAbove: Number(form.amountAbove) > 0 ? Number(form.amountAbove) : "" }); setBusy(false); if (ok) onClose(); }}>
    ...
    <label className="ta-legal-check"><input type="checkbox" checked={form.markReview} onChange={(event) => setForm({ ...form, markReview: event.target.checked })} /><span>{ui(locale, "Mark matching entries for review")}</span></label>
    <button className="tc-action" disabled={busy}>{ui(locale, busy ? "Saving…" : "Save rule")}</button>
  </form></Modal>;
}
...
export default function Cabinet({ onExit, onSignOut, user, onProfileUpdate, locale = "en", onLocale, theme = "dark", onTheme, nativeApp = false }) {
  ...
  const saveAutomationRule = async (form) => { const payload = { user_id: user.id, name: form.name.trim(), merchant_contains: form.merchantContains.trim() || null, amount_above: form.amountAbove === "" ? null : Number(form.amountAbove), entry_type: form.type || "any", action_entry_type: form.actionType || "keep", action_category: form.category || null, action_needs_review: Boolean(form.markReview), active: true, priority: automationRules.length + 1, updated_at: new Date().toISOString() }; const result = await supabase.from("automation_rules").insert(payload).select().single(); if (result.error) return setError(result.error); setAutomationRules((current) => [fromAutomationRule(result.data), ...current]); showToast(ui(locale, "Rule saved.")); return true; };
  ...
  else if (view === "automation") page = <AutomationPage rules={automationRules} canUse={plus} locale={locale} onAddPreset={saveAutomationRule} onNew={() => setModal("automation")} onToggle={toggleAutomationRule} onDelete={deleteAutomationRule} onUpgrade={() => setModal("pricing")} />;
  ...
  return <div className={`tc-app${nativeApp ? " tc-native" : ""}`}><aside className="tc-side"><button className="tc-brand" onClick={onExit}><i><ArrowUpRight size={17} /></i> Trek</button><small>{copy.shell.personal}</small>{localizedNav.map(([id, label, Icon]) => <button key={id} className={`${view === id ? "on" : ""}${lockedView(id) ? " locked" : ""}`} onClick={() => changeView(id)}><Icon size={17} /> {label}{lockedView(id) ? " · Plus" : ""}</button>)}<div className="tc-side-bottom"><button onClick={() => setModal("pricing")}><CreditCard size={17} /> {plan} {copy.shell.plan}</button><button onClick={() => setView("settings")}><Settings size={17} /> {copy.shell.settings}</button>{!nativeApp && <button onClick={onExit}><ArrowLeft size={16} /> {copy.shell.back}</button>}<button onClick={onSignOut}><LogOut size={17} /> {copy.shell.signout}</button></div></aside>
    ...
    {modal === "automation" && <AutomationModal locale={locale} onClose={() => setModal(null)} onSave={saveAutomationRule} />}
