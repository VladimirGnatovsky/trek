...
import { Bitcoin, ChevronRight, Coins, Pencil, Plus, RefreshCw, Trash2, TrendingDown, TrendingUp, X } from "lucide-react";
import "./crypto.css";
...
export function CryptoPage({ holdings, prices, history, period, loading, currency, hidden, plan, onPeriod, onAdd, onEdit, onDelete, onUpgrade, onRefresh, locale = "en" }) {
  ...
  return <section className="tc-page tcrypto-page">
    ...
    <section className="tc-panel tcrypto-list"><header><div><span>{ui(locale, "POSITIONS")}</span><h3>{ui(locale, "Your assets")}</h3></div>{plan === "Start" && <small>{holdings.length}/2 Start positions</small>}</header>{rows.length ? rows.map((row) => <article key={row.id}><span className="tcrypto-coin"><CryptoIcon symbol={row.symbol} size={34} /></span><div><b>{row.name}</b><small>{compact(row.quantity, hidden)} {row.symbol} · {ui(locale, "avg")} {amount(row.averageBuyPriceUsd * (row.quote.usdPrice ? row.quote.price / row.quote.usdPrice : 0), currency, hidden)}</small></div><div><b>{amount(row.value, currency, hidden)}</b><small className={row.quote.change24 >= 0 ? "positive" : "negative"}>{row.quote.change24 >= 0 ? <TrendingUp size={13} /> : <TrendingDown size={13} />}{Number(row.quote.change24 || 0).toFixed(2)}%</small></div><div><b className={row.pnl >= 0 ? "positive" : "negative"}>{row.pnl >= 0 ? "+" : ""}{amount(row.pnl, currency, hidden)}</b><small>{ui(locale, "unrealized P/L")}</small></div><div className="tcrypto-row-actions"><button onClick={() => onEdit({ ...row, averagePrice: row.averageBuyPriceUsd * (row.quote.usdPrice ? row.quote.price / row.quote.usdPrice : 0) })} aria-label={`${ui(locale, "Edit")} ${row.name}`}><Pencil size={15} /></button><button className="delete" onClick={() => onDelete(row.id)} aria-label={`Remove ${row.name}`}><Trash2 size={15} /></button></div></article>) : <div className="tcrypto-empty"><Coins size={27} /><b>{ui(locale, "No crypto assets yet")}</b><p>{ui(locale, "Add a position to see live value, performance and allocation.")}</p><button className="tc-action" onClick={onAdd}>{ui(locale, "Add your first asset")} <ChevronRight size={15} /></button></div>}</section>
...
export function CryptoModal({ currency, initial, onClose, onSave, locale = "en" }) {
  const [form, setForm] = useState(initial ? { ...initial, quantity: String(initial.quantity), averagePrice: String(initial.averagePrice || 0) } : { coinId: CRYPTO_CATALOG[0][0], quantity: "", averagePrice: "" });
  const [busy, setBusy] = useState(false);
  const selected = CRYPTO_CATALOG.find(([id]) => id === form.coinId) || CRYPTO_CATALOG[0];
  return <div className="tc-modal" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="tc-modal-card tcrypto-modal" role="dialog" aria-modal="true"><button className="tc-close" onClick={onClose} aria-label="Close"><X size={18} /></button><span className="tc-kicker">{ui(locale, initial ? "EDIT POSITION" : "NEW POSITION")}</span><h2>{ui(locale, initial ? "Edit crypto asset" : "Add a crypto asset")}</h2><p className="tc-modal-copy">{ui(locale, "Enter the amount you own and your average purchase price. Trek stores the position, never wallet credentials.")}</p><form className="tc-form" onSubmit={async (event) => { event.preventDefault(); setBusy(true); const ok = await onSave({ id: initial?.id, coinId: selected[0], symbol: selected[1], name: selected[2], quantity: Number(form.quantity.replace(",", ".")), averagePrice: Number(form.averagePrice.replace(",", ".")) }); setBusy(false); if (ok) onClose(); }}><label>{ui(locale, "Asset")}<select value={form.coinId} disabled={Boolean(initial)} onChange={(event) => setForm({ ...form, coinId: event.target.value })}>{CRYPTO_CATALOG.map(([id, symbol, name]) => <option key={id} value={id}>{symbol} · {name}</option>)}</select></label><label>{ui(locale, "Quantity")}<input required min="0.00000001" step="any" inputMode="decimal" value={form.quantity} onChange={(event) => setForm({ ...form, quantity: event.target.value })} placeholder="0.00" /></label><label>{ui(locale, "Average purchase price")} ({currency})<input required min="0" step="any" inputMode="decimal" value={form.averagePrice} onChange={(event) => setForm({ ...form, averagePrice: event.target.value })} placeholder={ui(locale, "Price per coin")} /></label><button className="tc-action" disabled={busy}>{busy ? ui(locale, "Saving…") : ui(locale, initial ? "Save changes" : "Save position")}</button></form></section></div>;
}
