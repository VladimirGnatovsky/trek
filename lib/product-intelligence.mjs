...
export function applyAutomationRules(form, rules = []) {
  return [...rules].filter((rule) => rule.active !== false).sort((a, b) => Number(b.priority || 0) - Number(a.priority || 0)).reduce((result, rule) => {
    ...
    const nextType = rule.actionType && rule.actionType !== "keep" ? rule.actionType : result.type;
    return {
      ...result,
      type: nextType,
      ...(rule.category && nextType !== "income" ? { category: rule.category } : {}),
      needsReview: Boolean(result.needsReview || rule.markReview),
    };
  }, { ...form });
}
